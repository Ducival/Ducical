...
[![codecov](https://codecov.io/gh/Ducival/Ducical/branch/main/graph/badge.svg?token=Ducical_token_here)](https://codecov.io/gh/Ducival/Ducical)
[![CI](https://github.com/Ducival/Ducical/actions/workflows/main.yml/badge.svg)](https://github.com/Ducival/Ducical/actions/workflows/main.yml)

Awesome ducical created by Ducival
...
[![Open in Gitpod](https://gitpod.io/button/open-in-gitpod.svg)](https://gitpod.io/#https://github.com/Ducival/Ducical)

### from source
```bash
git clone https://github.com/Ducival/Ducical ducical
cd ducical
make install
```
...
```bash
$ ducical run --port 8080
```

or

```bash
python -m ducical run --port 8080
```
...
```bash
❯ ducical --help
Usage: ducical [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ ducical create-user --help
Usage: ducical create-user [OPTIONS] USERNAME PASSWORD
...
```bash
ducical create-user admin admin --superuser
```
...
Run with `ducical run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `ducical create-user`.
...
-----------------------------------------------------
ducical/__init__.py              4      0   100%
ducical/app.py                  16      1    94%
ducical/cli.py                  21      0   100%
ducical/config.py                5      0   100%
ducical/db.py                   10      0   100%
ducical/models/__init__.py       0      0   100%
ducical/models/content.py       47      1    98%
ducical/routes/__init__.py      11      0   100%
ducical/routes/content.py       52     25    52%
ducical/routes/security.py      15      1    93%
ducical/routes/user.py          52     26    50%
ducical/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from ducical.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in ducical/default.toml.
...
```bash
PROJECT_NAME_ENV=production ducical run
```
